;; extends

(arguments (_) @swappable)
(array_expression (_) @swappable)
(declaration_list (_) @swappable)
(enum_variant) @swappable
(field_declaration) @swappable
(field_initializer_list (_) @swappable)
(function_item) @swappable
(match_arm) @swappable
(ordered_field_declaration_list (_) @swappable)
(parameters (_) @swappable)
(trait_bounds (_) @swappable)
(tuple_expression (_) @swappable)
(tuple_type (_) @swappable)
(type_arguments (_) @swappable)
(type_parameters (_) @swappable)
(use_list (_) @swappable)
(struct_item (_) @swappable)
(impl_item (_) @swappable)
(use_declaration (_) @swappable)
